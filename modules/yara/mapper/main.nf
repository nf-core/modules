...
process YARA_MAPPER {
    ...
    ${getProcessName(task.process)}:
        - ${getSoftwareName(task.process)}: \$(echo \$(yara_mapper --help  2>&1))
    END_VERSIONS
    """
    } else {
    """
    ...
    cat <<-END_VERSIONS > versions.yml
    ${getProcessName(task.process)}:
        - ${getSoftwareName(task.process)}: \$(echo \$(yara_mapper --version  2>&1) | grep -e "yara_mapper version:" | sed 's/yara_mapper version: //g')
    END_VERSIONS
    """
    }
