process GRIDSS_GRIDSS {
    ...
    tuple val("${task.process}"), val('gridss'), eval("GeneratePonBedpe --version | sed 's/-gridss//'"), topic: versions, emit: versions_gridss
    ...
    def args = task.ext.args ?: ''
    def prefix = task.ext.prefix ?: "${meta.id}"
    def steps = args.contains("-s ")
        ? args.split('-s ')[-1].split(" ")[0]
        : args.contains("--steps ")
            ? args.split('--steps ')[-1].split(" ")[0]
            : "all"
