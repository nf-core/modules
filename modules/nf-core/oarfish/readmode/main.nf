process OARFISH_READMODE {
    ...
    def transcriptome = fasta.name.endsWith('.oar') ? "--index ${fasta}" :
                        fasta.name.endsWith('.fa') || fasta.name.endsWith('.fasta') ? "--annotated ${fasta}" : ''
    def novel_transcripts = novel ? "--novel ${novel}" : ""

    // check if valid seq_tech is provided
    def valid_seq_tech = ['pac-bio', 'pac-bio-hifi', 'ont-cdna', 'ont-drna']
    if (!valid_seq_tech.contains(seq_tech)) {
        error("Invalid seq_tech: ${seq_tech}. Valid options are: ${valid_seq_tech.join(', ')}")
    }
