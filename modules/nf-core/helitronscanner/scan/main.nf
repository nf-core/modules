process HELITRONSCANNER_SCAN {
    ...
    if (command !in ['head', 'tail']) {
        error("[HELITRONSCANNER_SCAN] command argument should be 'head' or 'tail'")
    }
    if (!task.memory) {
        error('[HELITRONSCANNER_SCAN] Available memory not known. Specify process memory requirements to fix this.')
    }

    def args = task.ext.args ?: ''
    def prefix = task.ext.prefix ?: "${meta.id}"

    def is_head = { command == 'head' }.call()
