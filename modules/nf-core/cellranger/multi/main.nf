process CELLRANGER_MULTI {
    ...
    path gex_frna_probeset , stageAs: "references/gex_probeset/*"
    path gex_targetpanel   , stageAs: "references/gex_targetpanel/*"
    path vdj_reference     , stageAs: "references/vdj/*"
    path vdj_primer_index  , stageAs: "references/vdj_primers/*"
    path fb_reference      , stageAs: "references/fb/*"
    path beam_panel        , stageAs: "references/beam/beam_panel.csv"
    path cmo_reference     , stageAs: "references/cmo/*"
    path cmo_barcodes      , stageAs: "references/cmo/cmo_barcodes.csv"
    path frna_sampleinfo   , stageAs: "references/frna/frna_sampleinfo.csv"
	...
	def gex_reference_name     = gex_reference.getName() != 'EMPTY'     ? gex_reference.getName() : ''
    println(gex_reference_name)
	def gex_frna_probeset_name = gex_frna_probeset.getName() != 'EMPTY' ? gex_frna_probeset.getName() : ''
    println(gex_frna_probeset_name)
	def gex_targetpanel_name   = gex_targetpanel.getName() != 'EMPTY'   ? gex_frna_probeset.getName() : ''
    println(gex_targetpanel_name)
	def vdj_reference_name     = vdj_reference.getName() != 'EMPTY'     ? vdj_reference.getName() : ''
    println(vdj_reference_name)

	// if references + fastqs are empty, then don't run corresponding analyses
	def include_gex    = gex_fastqs.first().getName() != 'EMPTY' && gex_reference          ? "[gene-expression]\nreference,references/gex/${gex_reference_name}" : ''
	def include_vdj    = vdj_fastqs.first().getName() != 'EMPTY' && vdj_reference          ? "[vdj]\nreference,references/vdj/${vdj_reference_name}" : ''
	def include_fb     = fb_reference.first().getName() != 'EMPTY'                         ? '[feature]' : ''
	def include_beam   = beam_fastqs.first().getName() != 'EMPTY' && beam_panel            ? '[antigen-specificity]' : ''
	def include_cmo    = cmo_fastqs.first().getName() != 'EMPTY' && cmo_barcodes           ? '[samples]' : ''
	def include_frna   = gex_frna_probeset.first().getName() != 'EMPTY' && frna_sampleinfo ? '[samples]' : ''

	// targeted GEX panel goes under GEX section, not its own
	def target_panel   = gex_targetpanel.getName() != 'EMPTY' ? "target-panel,\$PWD/references/gex_targetpanel" : '' 

	// fixed RNA reference (not sample info!) also goes under GEX section
	def frna_probeset  = gex_frna_probeset.getName() != 'EMPTY' ? "probe-set,\$PWD/references/gex_probeset" : ''

	// VDJ inner primer set
	def primer_index   = vdj_primer_index.getName() != 'EMPTY' ? "inner-enrichment-primers,\$PWD/references/vdj_primers" : '' 
	...
	def beam_csv_text = include_beam    ? beam_panel_csv.text : ''
	def cmo_csv_text  = include_cmo     ? cmo_barcode_csv.text : ''
	def frna_csv_text = include_frna    ? frna_sampleinfo_csv.text : ''
	...
	def fastq_antibody = include_fb     ? "${meta_ab.id},\$PWD/fastqs/ab,Antibody Capture" : ''
	def fastq_beam     = include_beam   ? "${meta_beam.id},\$PWD/fastqs/beam,Antigen Capture" : ''
	def fastq_crispr   = include_fb     ? "${meta_crispr.id},\$PWD/fastqs/crispr,CRISPR Guide Capture" : ''
