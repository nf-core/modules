process CELLRANGER_MULTI {
    ...
    touch $config
    if [ "$include_gex" ]; then
        echo "$include_gex" >> $config
        echo "$gex_reference_path" >> $config
        echo "$gex_options" >> $config
    ...
    if [ "$include_vdj" ]; then
        echo "$include_vdj" >> $config
        echo "$vdj_reference_path" >> $config
        echo "$vdj_options" >> $config
        if [ "$primer_index" ]; then echo "$primer_index" >> $config; fi
    fi
    if [ "$include_fb" ]; then
        echo "$include_fb" >> $config
        echo "$fb_options" >> $config
    fi
    if [ "$include_beam" ]; then
        echo "$include_beam" >> $config
        echo "$beam_csv_text" >> $config
    fi
    if [ "$include_cmo" ]; then
        echo "$include_cmo" >> $config
        echo "$cmo_csv_text" >> $config
    fi
    if [ "$include_frna" ]; then
        echo "$include_frna" >> $config
        echo "$frna_csv_text" >> $config
    fi
