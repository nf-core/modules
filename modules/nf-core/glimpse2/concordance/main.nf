process GLIMPSE2_CONCORDANCE {
    ...
    if (((groups ? 1 : 0) + (bins ? 1 : 0) + (ac_bins ? 1 : 0) + (allele_counts ? 1 : 0)) != 1) {
        error("One and only one argument should be selected between groups, bins, ac_bins, allele_counts")
    }
    ...
    def rsquare_per_site_cmd = args.contains("--out-r2-per-site") ? "echo | gzip > ${prefix}_r2_sites.txt.gz" : ""
