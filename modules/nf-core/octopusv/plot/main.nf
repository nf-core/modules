process OCTOPUSV_PLOT {
    ...
    tuple val(meta), path("${prefix}_sv_sizes.svg"), emit: sv_sizes_svg, optional: true
    tuple val(meta), path("${prefix}_sv_types.png"), emit: sv_types_png
    tuple val(meta), path("${prefix}_sv_types.svg"), emit: sv_types_svg, optional: true
    tuple val(meta), path("${prefix}_chromosome_distribution.png"), emit: chromosome_distribution_png
    tuple val(meta), path("${prefix}_chromosome_distribution.svg"), emit: chromosome_distribution_svg, optional: true
    ...
    def args = task.ext.args ?: ''
    prefix = task.ext.prefix ?: "${meta.id}"
    def svg_outputs = args.contains('--no-svg') ? '' : """
