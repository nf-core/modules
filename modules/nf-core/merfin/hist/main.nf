process MERFIN_HIST {
    ...
    tuple val(meta2), path(meryl_db_reads)  // Required readmers (raw reads meryl db). As it comes from another tool, it might be relevant to maintain the meta.
    ...
    tuple val("${task.process}"), val("merfin"), eval("merfin --version |& sed 's/merfin //'"), emit: versions_merfin, topic: versions
