process WIPERTOOLS_FASTQWIPER {
    ...
    output:
    tuple val(meta), path("${prefix}.fastq.gz"), emit: wiped_fastq  // channel: [ val(meta), *_wiped.fastq | *_wiped.fastq.gz ]
    tuple val(meta), path("*.report")          , emit: report       // channel: [ val(meta), *.report ]
