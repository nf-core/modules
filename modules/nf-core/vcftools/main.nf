process VCFTOOLS {
    ...
    def bed_arg = args.contains('--bed')
        ? "--bed ${bed}"
        : args.contains('--exclude-bed')
            ? "--exclude-bed ${bed}"
            : args.contains('--hapcount')
                ? "--hapcount ${bed}"
                : args.contains('--positions')
                    ? "--positions ${bed}"
    ...
    def diff_variant_arg = args.contains('--diff')
        ? "--diff ${diff_variant_file}"
        : args.contains('--gzdiff')
            ? "--gzdiff ${diff_variant_file}"
    ...
    def input_file = "${variant_file}".endsWith(".vcf")
        ? "--vcf ${variant_file}"
        : "${variant_file}".endsWith(".vcf.gz")
            ? "--gzvcf ${variant_file}"
