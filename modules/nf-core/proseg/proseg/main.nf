process PROSEG {
    ...
    tuple val(meta), path("${prefix}_transcript-metadata.{csv,csv.gz,parquet}"), emit: transcript_metadata
    tuple val(meta), path("${prefix}_cell-polygons.geojson.gz"                ), emit: cell_polygons
    tuple val(meta), path("${prefix}_cell-metadata.{csv,csv.gz,parquet}"      ), emit: cell_metadata
    tuple val(meta), path("${prefix}_cell-polygons-layers.geojson.gz"         ), emit: cell_polygons_layers
    tuple val(meta), path("${prefix}_expected-counts.{csv,csv.gz,parquet}"    ), emit: expected_counts
    tuple val(meta), path("${prefix}_cell-polygons-union.geojson.gz"          ), emit: union_cell_polygons
