process DEEPVARIANT_POSTPROCESSVARIANTS {
    ...
    tuple val("${task.process}"), val('deepvariant_postprocessvariants'), eval("/opt/deepvariant/bin/run_deepvariant --version | sed 's/^.*version //; s/ .*\$//'"), topic: versions, emit: versions_deepvariant_postprocessvariants
