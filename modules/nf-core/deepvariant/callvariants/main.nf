...
process DEEPVARIANT_CALLVARIANTS {
    ...
    tuple val("${task.process}"), val('deepvariant_callvariants'), eval("/opt/deepvariant/bin/run_deepvariant --version | sed 's/^.*version //; s/ .*\$//'"), topic: versions, emit: versions_deepvariant_callvariants
