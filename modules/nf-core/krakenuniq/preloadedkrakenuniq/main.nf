process KRAKENUNIQ_PRELOADEDKRAKENUNIQ {
    ...
    classified = meta.single_end ? "\${PREFIX}.se.classified.${sequence_type}" : "\${PREFIX}.pe.merged.classified.${sequence_type}"
    unclassified = meta.single_end ? "\${PREFIX}.se.unclassified.${sequence_type}" : "\${PREFIX}.pe.merged.unclassified.${sequence_type}"
    ...
    def output_option = save_output && meta.single_end ? '--output "\${PREFIX}.se.krakenuniq.classified.txt"' : save_output && !meta.single_end ? '--output "\${PREFIX}.pe.krakenuniq.classified.txt"' : ''
    def report = report_file && meta.single_end ? '--report-file "\${PREFIX}.se.krakenuniq.report.txt"' : report_file && !meta.single_end ? '--report-file "\${PREFIX}.pe.krakenuniq.report.txt"' : ''
    ...
    if (meta.single_end) {
        ...
        while IFS='\t' read -r SEQ PREFIX; do
            ...
            create_file "\${PREFIX}.se.krakenuniq.classified.txt"
            create_file "\${PREFIX}.se.krakenuniq.report.txt"
            create_gzip_file "\${PREFIX}.se.classified.${sequence_type}.gz"
            create_gzip_file "\${PREFIX}.se.unclassified.${sequence_type}.gz"
        done < ${command_inputs_file}
        ...
        while IFS='\t' read -r FIRST_SEQ SECOND_SEQ PREFIX; do
            ...
            create_file "\${PREFIX}.pe.krakenuniq.classified.txt"
            create_file "\${PREFIX}.pe.krakenuniq.report.txt"
            create_gzip_file "\${PREFIX}.pe.merged.classified.${sequence_type}.gz"
            create_gzip_file "\${PREFIX}.pe.merged.unclassified.${sequence_type}.gz"
        done < ${command_inputs_file}
