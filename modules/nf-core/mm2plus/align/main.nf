process MM2PLUS_ALIGN {
    tag "$meta.id"
    label 'process_high'

    // Note: the versions here need to match the versions used in the mulled container below and mm2plus/index
    ...
    def target = reference ?: (bam_input ? error("Error: mm2-plus/align BAM input mode requires reference") : reads)
    """
    ...
    if(bam_input && !reference) {
        error("Error: mm2-plus/align BAM input mode requires reference!")
	}
    """
    touch $output_file
    ${bam_index}
