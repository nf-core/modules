process BISCUIT_PILEUP {
    ...
    def biscuit_cpus = [(task.cpus * 0.9) as Integer, 1].max()
    def bgzip_cpus = task.cpus - biscuit_cpus < 1 ? biscuit_cpus : (task.cpus - biscuit_cpus)
    if (tumor_bam != [] && normal_bams.toList().size() > 1) {
        error("[BISCUIT_PILEUP] error: Tumor BAM provided with more than one normal BAM")
    }
    if (tumor_bam.toList().size() > 1) {
        error("[BISCUIT_PILEUP] error: more than one tumor BAM provided")
    }
