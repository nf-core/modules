...
workflow FASTA_INDEX_DNA {
    ...
    if (val_aligner == 'bowtie2') {
        ...
        ch_aligner_index = ch_aligner_index.mix(
            BWAMEM1_INDEX.out.index.join(ch_altliftover).map { meta, index, alt -> [meta, index + alt] }
        )
        ...
        ch_aligner_index = ch_aligner_index.mix(
            BWAMEM2_INDEX.out.index.join(ch_altliftover).map { meta, index, alt -> [meta, index + alt] }
        )
    }
