...
workflow BAM_TUMOR_NORMAL_SOMATIC_VARIANT_CALLING_GATK {
    ...
    emit:
    artifact_priors     = GATK4_LEARNREADORIENTATIONMODEL.out.artifactprior.collect() // channel: [ val(meta), path(artifactprior) ]
    contamination_table = GATK4_CALCULATECONTAMINATION.out.contamination.collect() // channel: [ val(meta), path(table) ]
    filtered_stats      = GATK4_FILTERMUTECTCALLS.out.stats.collect() // channel: [ val(meta), path(stats) ]
    filtered_tbi        = GATK4_FILTERMUTECTCALLS.out.tbi.collect() // channel: [ val(meta), path(tbi) ]
    filtered_vcf        = GATK4_FILTERMUTECTCALLS.out.vcf.collect() // channel: [ val(meta), path(vcf) ]
    mutect2_f1r2        = GATK4_MUTECT2.out.f1r2.collect() // channel: [ val(meta), path(f1r2) ]
    mutect2_stats       = GATK4_MUTECT2.out.stats.collect() // channel: [ val(meta), path(stats) ]
    mutect2_tbi         = GATK4_MUTECT2.out.tbi.collect() // channel: [ val(meta), path(tbi) ]
    mutect2_vcf         = GATK4_MUTECT2.out.vcf.collect() // channel: [ val(meta), path(vcf) ]
    pileup_table_normal = GATK4_GETPILEUPSUMMARIES_NORMAL.out.table.collect() // channel: [ val(meta), path(table) ]
    pileup_table_tumor  = GATK4_GETPILEUPSUMMARIES_TUMOR.out.table.collect() // channel: [ val(meta), path(table) ]
    segmentation_table  = GATK4_CALCULATECONTAMINATION.out.segmentation.collect() // channel: [ val(meta), path(table) ]
