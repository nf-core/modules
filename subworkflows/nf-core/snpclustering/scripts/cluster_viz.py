...
def _normalise_id_column(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Rimuovi riga header duplicata (IID == "FID" o "IID")
...
def plot_scatter(df, x, y, out_png, title, xlabel=None, ylabel=None):
    ...
    sc = plt.scatter(df[x], df[y], c=labels, cmap="Paired", s=18, alpha=0.85)
    plt.title(title)
...
def main() -> None:
    ...
    plot_scatter(tsne_df, "x", "y", args.out_tsne_png,
                 f"t-SNE (perplexity={perp:.1f}) colored by cluster")

    pca_df = pd.read_csv(args.pca_scores, sep="\t", dtype=str)
    ...
    plot_scatter(merged, c1, c2, args.out_pca_png,
                 "PCA (first two components) colored by cluster", c1, c2)
