...
workflow BAM_DEDUP_STATS_SAMTOOLS_UMITOOLS {
    ...
    if (val_primary_only) {
        ...
        ch_dedup_input = SAMTOOLS_VIEW_PRIMARY.out.bam.join(
            SAMTOOLS_INDEX_PRIMARY.out.bai.mix(SAMTOOLS_INDEX_PRIMARY.out.csi),
            by: [0],
            remainder: true,
        )
    }
    ...
    ch_bam_bai_dedup = UMITOOLS_DEDUP.out.bam.join(
        SAMTOOLS_INDEX.out.bai.mix(SAMTOOLS_INDEX.out.csi),
        by: [0],
        remainder: true,
    )
