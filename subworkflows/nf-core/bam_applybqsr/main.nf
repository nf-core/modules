...
workflow BAM_APPLYBQSR {
    ...
    // Combine single and merged multiple bam and index files, removing num_intervals field
        bam_recal = bam_all
            ...
            .map { meta, bam_, bai -> [meta - meta.subMap('num_intervals'), bam, bai] }

    // FOR CRAMs

    // ---- CRAM path ----
    // Branch CRAM fragments (from ApplyBQSR) by number of intervals
    cram_applybqsr_out = GATK4_APPLYBQSR_CRAM.out.cram
        .join(GATK4_APPLYBQSR_CRAM.out.bai, failOnDuplicate: true, failOnMismatch: true)
        .branch {
            single:   it[0].num_intervals == 1
            multiple: it[0].num_intervals > 1
        }
    ...
    // Gather fragments (files + indexes) for samtools merge
    ...
    // Merged CRAMs carry their index on .index; recombine with single-interval ones, drop num_intervals
    cram_recal = MERGE_CRAM.out.cram
        .join(MERGE_CRAM.out.index, failOnDuplicate: true, failOnMismatch: true)
        .mix(cram_applybqsr_single)
