...
workflow FASTA_NEWICK_EPANG_GAPPA {
    ...
    ch_epang_query = ch_pp_data
        ...
        .mix(
            ch_pp_data.map { it -> [it.meta, it.data.model, it.data.refphylogeny] }.join(
                EPANG_SPLIT_CLUSTALO.out.query.mix(EPANG_SPLIT_MAFFT.out.query).map { it -> [it[0], it[1]] }
            ).join(
                EPANG_SPLIT_CLUSTALO.out.reference.mix(EPANG_SPLIT_MAFFT.out.reference).map { it -> [it[0], it[1]] }
            )
        )
