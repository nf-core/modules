...
workflow GATK_TUMOR_NORMAL_SOMATIC_VARIANT_CALLING {
    ...
    GATK_MUTECT2 ( mutect2_input, false, false, false, [], fasta, fai, dict, germline_resource, germline_resource_tbi, panel_of_normals, panel_of_normals_tbi )
    ...
    //
    ch_learnread_in = GATK_MUTECT2.out.f1r2.collect()
    GATK_LEARNREADORIENTATIONMODEL (ch_learnread_in)
    ...
    //
    ch_vcf           = GATK_MUTECT2.out.vcf.collect()
    ch_tbi           = GATK_MUTECT2.out.tbi.collect()
    ch_stats         = GATK_MUTECT2.out.stats.collect()
    ch_orientation   = GATK_LEARNREADORIENTATIONMODEL.out.artifactprior.collect()
    ...
    mutect2_tbi         = GATK_MUTECT2.out.tbi.collect()                             // channel: [ val(meta), [ tbi ] ]
    mutect2_stats       = GATK_MUTECT2.out.stats.collect()                           // channel: [ val(meta), [ stats ] ]
    mutect2_f1r2        = GATK_MUTECT2.out.f1r2.collect()                            // channel: [ val(meta), [ f1r2 ] ]

    artifact_priors     = GATK_LEARNREADORIENTATIONMODEL.out.artifactprior.collect() // channel: [ val(meta), [ artifactprior ] ]
    ...
    filtered_tbi        = GATK_FILTERMUTECTCALLS.out.tbi.collect()                   // channel: [ val(meta), [ tbi ] ]
