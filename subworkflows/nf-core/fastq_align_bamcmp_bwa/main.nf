...
workflow FASTQ_ALIGN_BAMCMP_BWA {
    ...
    BAM_SORT_STATS_SAMTOOLS_UNFILTERED(BWA_MEM_PRIMARY.out.bam, ch_fasta_fai, 'bai')

    //
    // Sort, index filtered BAM file and run samtools stats, flagstat and idxstats
    //

    BAM_SORT_STATS_SAMTOOLS_FILTERED(BAMCMP.out.primary_filtered_bam, ch_fasta_fai, 'bai')
